...
Feature: Merchandise Business unit Setup page
  ...
  Background: 
    ...
    And User Can Select Company "ackermans"
  ...
  Scenario: Merchandise setup from Business unit to subclass
    ...
    When User enter business Names
   	And The user select Reporting category "REPORT222" 
		And The user select Alternative Hierarchy "902166#ALTH SK#TEST222"
    And verify that the Active status is selected by default from the Status dropdown
    And submit the page
    ...
    And The user select Reporting category "REPORT222" 
    And The user select Alternative Hierarchy "902166#ALTH SK"
    ...
    And The user select Reporting category "REP-1314"
    And User select item Type "AutomationTest"
    And The user select Alternative Hierarchy "902166#ALTH SK"
    And verify that the Active status is selected by default from the Status dropdown
    And submit the page
